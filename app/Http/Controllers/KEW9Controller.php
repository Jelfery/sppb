...
class KEW9Controller extends Controller
{
    ...
    public function index(){
    	...
    	return View::make('KEW9.index', compact('records'));
    }
    ...
    public function store(Request $request){
        ...
        $filename = $user->hospital->short_name.'_KEW9_'.$short_month.$year.'.'.$file->getClientOriginalExtension();

        // Move Uploaded File
        $destinationPath = 'records/KEW9/'.Carbon::now()->year.'/'.$full_month;
    ...
    public function get($id){
    	...
    	// PDF file is stored under sppb/records/KEW9
    	$path = public_path(). "/records/KEW9/".$year.'/'.$month.'/'.$file->name;
    ...
    public function getCollection(Request $request){
        ...
        $path = public_path(). "/records/KEW9/".$request->year;
        ...
        if (File::isDirectory($path)) {
            ...
            $filename = 'KEW9_'.$request->year.$file_extension;

            // path to the new zip file
            $zipPath = public_path()."/records/KEW9/collection/".$filename;
